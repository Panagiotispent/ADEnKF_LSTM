...
class LSTMLayer(nn.Module):
    ...
    def forward(self, input, state, noise):
        ...
        for i in range(len(inputs)):
            out, state = self.cell(inputs[i], state, noise)
            outputs += [out]
...
class ModelH(nn.Module):
    def __init__(self, target, hidden_units):
        ...
        self.output = target
        # linear layer
        self.H = nn.Linear(in_features=self.hidden_units, out_features=len([target]),dtype=torch.double)
        # self.H_out = nn.Linear(in_features=self.hidden_units, out_features=len([target]),dtype=torch.double)
